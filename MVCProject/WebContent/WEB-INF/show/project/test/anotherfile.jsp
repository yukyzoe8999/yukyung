...
<body>
	<form action="anotherfile">
		<select name="f">
			<option ${(param.f == "Name")?"selected":""} value="NAME">이름</option>
		</select> <label>검색</label> <input type="text" name="q" value="${param.q}" /> 
	...
	<form method="post">
		...
		<c:forEach var="t" items="${test}" varStatus="vs">
			...
			<tr>
				...
				<td><input type = "checkbox" name = "try" value = "${t.seq}" ></td>
                <td><input type = "checkbox" name = "delete" value = "${t.seq}" ></td>

			</tr>
		</c:forEach>
	</table>
	
	<input type="submit" name = "cmd" value="TryAll" />
	<input type="submit" name = "cmd" value="DeleteAll" />
