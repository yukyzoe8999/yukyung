...
//				System.out.println(p_dex);
			...
			while (tour) {
				...
				if(p_hp <= ((p_hp*50)/100)) {
					System.out.println("체려이 너무 낮습니다.");
				}
				...
				case 1:
					...
					if (monster_encounter > 4) {
						...
						monster_kind = rdm.nextInt((dice_kind+1)); // 주사위를 굴려 몬스터 종류를 정
						...
						if (de == 1) {
							while (monster_battle) {
								int hitrating = rdm.nextInt((hitrating_dice+1));
